  ...
  <view class="page-container">
    <view class="add-form">
      <u-form :model="formData" ref="uForm" :label-width="140">
        <u-form-item label="亲友姓名" prop="name"
          ><u-input v-model="formData.name" placeholder="请输入亲友姓名"
        /></u-form-item>
        <u-form-item
        ...
        >
          <u-input
            ...
            v-model="selectBelongLabel"
            placeholder="请选择TA属于谁"
        ...
        >
          <u-input
            type="select"
            :select-open="sexSelectState"
            v-model="selectSexLable"
            placeholder="请选择性别"
            @click="sexSelectState = true"
          />
        </u-form-item>
        <u-form-item label="关系备注"
    ...
    (this.$refs.uForm as any).validate((valid: any) => {
      if (valid) {
        ...
        friendApi.add(this.formData).then((res) => {
          console.log(res);
          if (res) {
            (this as any).goTo("/pages/user/friends/index");
          }
        });
      } else {
        console.log("验证失败");
